...
export async function storeUserStatus(userId, rawInput, processedStatus, kvStore, historyLimit = 20) {
  ...
  try {
    ...
    const historyData = await kvStore.get(historyKey, { type: 'json' })
 ...
 * This includes status history, latest status, profile, and interaction logs.
...
export async function purgeUserData(userId, kvStore) {
  ...
  const keyPrefixes = [
    `user:${userId}:`,      // Covers status_history, latest_status, activities (if added later)
...
export async function logUserInteraction(targetUserId, actorUserId, interactionType, timestamp, kvStore, logLimit = 100) {
  ...
  try {
    ...
    const interactions = await kvStore.get(key, { type: 'json' })
