...
export async function updateUserProfile(userId, updates, kvStore) {
  ...
  try {
    ...
    const profile = await getUserProfile(userId, kvStore) // getUserProfile handles creation if needed
...
export async function setCustomEmoji(userId, stateName, emoji, kvStore) {
  ...
  try {
    const profile = await getUserProfile(userId, kvStore) // Get or create profile
